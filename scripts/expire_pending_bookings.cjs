/**
 * One-off migration script (CommonJS) to mark pending bookingRequests whose start time is in the past
 ...
 *  - Run: node scripts/expire_pending_bookings.cjs
 */
...
async function main() {
  ...
  snapshot.forEach(doc => {
    ...
    let startMs = null;
    if (data.startAt && typeof data.startAt.toMillis === 'function') {
