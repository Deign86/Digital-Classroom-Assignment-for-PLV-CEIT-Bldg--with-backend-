...
export const handlers = [
	...
	rest.post('/__mock_functions/createNotification', async (req, res, ctx) => {
	...
	rest.post('/__mock_functions/acknowledgeNotification', async (req, res, ctx) => {
		return res(ctx.status(200), ctx.json({ success: true }));
	}),

	// Simulate bulk acknowledge
	rest.post('/__mock_functions/acknowledgeNotifications', async (req, res, ctx) => {
	...
	rest.get('/__mock_api/notifications', (req, res, ctx) => {
