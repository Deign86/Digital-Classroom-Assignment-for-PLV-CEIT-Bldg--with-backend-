          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button variant="destructive" className="rounded-full" onClick={async () => {
                ...
                setIsPromoting(true);
                const target = pendingPromotionUser;
                try {
                  if (onChangeRole) {
                    // allow onChangeRole to optionally return a structured response { success, message, notifyCurrentlyLoggedIn }
                    const res: any = await (onChangeRole as any)(target.id, 'admin');
                    if (res && res.message) {
                      toast.success(res.message);
                    } else {
                      toast.success(`${target.name} is now an admin.`);
                    }
                    // If backend indicates the user is currently logged in, prompt admin to notify them to re-login
                    if (res && res.notifyCurrentlyLoggedIn) {
                      setNotifyTargetUser(target);
                    }
                  }
                } catch (err: any) {
          ...
          <DialogContent>
            ...
            <DialogFooter>
              ...
              <Button variant="destructive" className="rounded-full" onClick={async () => {
                ...
                setIsDemoting(true);
                const target = pendingDemotionUser;
                try {
                  if (onChangeRole) {
                    const res: any = await (onChangeRole as any)(target.id, 'faculty');
                    if (res && res.message) {
                      toast.success(res.message);
                    } else {
                      toast.success(`${target.name} is now a faculty.`);
                    }
                    if (res && res.notifyCurrentlyLoggedIn) {
                      setNotifyTargetUser(target);
                    }
